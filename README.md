...
매 에피소드 = **5문제 × 24초**. 한 문제 24초는 다음 4구간으로 구성:

| 구간 | 길이 | 내용 |
| --- | ---:| --- |
| 퀴즈 제시 | 5s | OX 또는 4지선다 문제 — nova 나레이션 + 큰 자막 |
| 카운트다운 | 3s | 3→2→1 (비프음 3회, 마지막 길고 낮게 강조 · 매 초 밝기 펄스) |
| 정답 공개 | 3s | 화면 플래시 + "정답: O / X / N번" 크게 + onyx 음성 |
| 해설 | 13s | shimmer 음성으로 정답 근거 + 핵심 맥락 간결하게 |

각 문제 사이에는 0.35s 화이트 플래시 전환. 문제 번호 배지(`문제 1/5` 등)는 상단 좌측에 계속 표시.
