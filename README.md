...
## 영상 포맷 (132초 · 5문제 챌린지 + CTA)

매 에피소드 = **5문제 × 24초 + CTA 12초 = 132초**. 한 문제 24초 구성:

| 구간 | 길이 | 내용 |
| --- | ---:| --- |
| 퀴즈 제시 | 5s | OX 또는 4지선다 — nova(여) 나레이션 + 큰 자막 (보기는 자막 전용) |
| 카운트다운 | 3s | 3→2→1 (비프음 3회, 마지막 길고 낮게 강조) |
| 정답 공개 | 3s | 화면 플래시 + 큰 자막 + **onyx(남) 음성만** |
| 해설 | 13s | shimmer(여) — 근거·맥락 간결하게 ("정답은" 시작 금지) |

**CTA 아웃트로 (12s, 120~132s)** — onyx(남) 낭독 + 3개 배지 각 4초:
- 120-124s: `👍 좋아요`
- 124-128s: `✔ 구독`
- 128-132s: `🔔 알림설정`
- TTS 멘트: "좋아요, 구독, 알림 설정 부탁드립니다!"

**상단 고정** — 전체 132초 동안 `오! 당신은 역사퀴즈왕` 채널 헤더 (90pt 노란색 + 검은 외곽선).
각 문제 사이: 0.35s 화이트 플래시. 문제 번호 배지(`문제 1/5`)는 상단 좌측.
