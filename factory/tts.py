...
# ── 역사 고유 명칭(날짜형) 한글 표기 매핑 ─────────────────────────────────────
# TTS 가 "3.1 운동" 을 "삼 점 일 운동" 으로 어색하게 읽는 문제를 막기 위한 사전 치환.
_HISTORICAL_DATE_TERMS: list[tuple[str, str]] = [
    ("3.1운동",   "삼일운동"),
    ("3·1운동",   "삼일운동"),
    ("3.1 운동",  "삼일 운동"),
    ("3·1 운동",  "삼일 운동"),
...
def _convert_numbers(text: str) -> str:
    # 1단계: 역사 고유 표기 선치환
    for src, dst in _HISTORICAL_DATE_TERMS:
        text = text.replace(src, dst)
    # 2단계: 남은 숫자를 한국어(한자식)로 변환
